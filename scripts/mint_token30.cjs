...
async function main() {
    ...
    // Scegliamo tokenId 30, come richiesto, e quantità 1.
    const TOKEN_ID_TO_MINT = 30; 
    const QUANTITY_TO_MINT = 1; 

    // Calcola il valore esatto da inviare (in Wei) per il contratto nuovo e tokenId 30
    // Prezzo per tokenId 'i' è 0.04 MATIC * i
    // Per tokenId 30, il prezzo è 0.04 * 30 = 1.2 MATIC
    const MINT_VALUE_MATIC = 1.2; 
    const MINT_VALUE_WEI = ethers.parseEther(MINT_VALUE_MATIC.toString()); // Converti in Wei (BigInt)

    console.log("------------------------------------------");
    console.log("Inizio l'operazione di Mint del Token ID 30 sul contratto LHISA_LecceNFT...");
    console.log("------------------------------------------");
