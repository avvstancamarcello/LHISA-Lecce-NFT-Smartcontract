// scripts/mint_token_60.cjs
require("dotenv").config();
const { ethers } = require("hardhat");

async function main() {
    const [signer] = await ethers.getSigners(); // Il firmatario di questa transazione (dal tuo .env)
    ...
    // Scegliamo tokenId 60, come richiesto, e quantità 1.
    const TOKEN_ID_TO_MINT = 60; 
    const QUANTITY_TO_MINT = 1; 

    // Calcola il valore esatto da inviare (in Wei) per il contratto nuovo e tokenId 60
    // Prezzo per tokenId 'i' è 0.04 MATIC * i
    // Per tokenId 60, il prezzo è 0.04 * 60 = 2.4 MATIC
    const MINT_VALUE_MATIC = 2.4; 
    const MINT_VALUE_WEI = ethers.parseEther(MINT_VALUE_MATIC.toString()); // Converti in Wei (BigInt)

    console.log("------------------------------------------");
    console.log("Inizio l'operazione di Mint del Token ID 60 sul contratto LHISA_LecceNFT...");
    console.log("------------------------------------------");
